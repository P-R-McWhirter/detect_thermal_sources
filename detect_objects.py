...
for file in imgs:
	...
	for c in cnts:
		...
		## Record an image of the RoI area.

		roicol = orisave[(y):(y+h), (x):(x+w)]
		roi = cv2.cvtColor(roicol, cv2.COLOR_BGR2GRAY)
		...
		## RoIs with keep = True are recorded into the roi subfolder with an associated number (z).
		...
		if keep is True:
			...
			ww = w / sizex
			wh = h / sizey
			#cv2.imwrite(cwd + "/roi/" + file[:-4] + "-roi-" + str(z) + ".jpg", roicol)
			roifile.write("0 " + str(wx) + " " + str(wy) + " " + str(ww) + " " + str(wh) + "\n")
			cv2.rectangle(ori, (x,y), (x+w, y+h), (0, 255, 0), 2)

	roifile.close()

	cv2.imwrite(cwd + "/box/" + file, ori)
